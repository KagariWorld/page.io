...
(function($){
  "use strict";
	...
	var creativesNFTLeo = {
		...
		init: function(){
			creativesNFTLeo.animatedText();
			creativesNFTLeo.imgToSVG();
			creativesNFTLeo.BgImg();
			
			
			creativesNFTLeo.headerTrigger();
			creativesNFTLeo.menuFixer();
			creativesNFTLeo.timeLine();
			creativesNFTLeo.movingBlog();	
			creativesNFTLeo.navFixer();
			creativesNFTLeo.anchor();
			creativesNFTLeo.movingTextForNav();
			creativesNFTLeo.filterItems();
			creativesNFTLeo.applyFilter();
			creativesNFTLeo.magnific();
			creativesNFTLeo.progressTotop();
			creativesNFTLeo.productModal();
			creativesNFTLeo.contactForm();
			creativesNFTLeo.moreCategories();
			creativesNFTLeo.totop();
			creativesNFTLeo.vegasSlider();
			creativesNFTLeo.heroJarallaxSlider();
			creativesNFTLeo.subscribe();
		},
		...
		subscribe: function(){
			$(".subscribe_form a").on('click', function(){
				...
				if(email === ''){
					...
					if(creativesNFTLeo.validateEmail(email)){
		...
		vegasSlider: function(){
			$('.NFTLeo_fn_hero .vegas-slide').each(function(){
				var element = $(this);
				var html	= []; 
		...
		heroJarallaxSlider: function(){
			// slider
			var section		= $('.NFTLeo_fn_hero .swiper-container');
		...
		totop: function (){
			var minSpeed 		= 500;
			var maxSpeed		= 1500;
			$(".NFTLeo_fn_totop").off().on('click', function(e) {
				e.preventDefault();
		...
		moreCategories: function(){
			$('.NFTLeo_fn_categories').each(function(){
				var element 	= $(this);
			...
			creativesNFTLeo.showMore();
		},
		
		showMore: function(){
			$('.NFTLeo_fn_categories .more').off().on('click',function(){
				var element		= $(this),
					parent		= element.closest('.NFTLeo_fn_categories'),
		...
		productModal: function(){
			var modal = $('.NFTLeo_fn_modal.product_modal');
			$('.NFTLeo_fn_drops .item a').off().on('click', function(){
				var element		= $(this);
				...
				modal.find('.NFTLeo_fn_title .fn_title').text(title);
		...
		applyFilter: function(){
			...
			creativesNFTLeo.isotopeCollection();
			
			// left filter on click function
			$('.NFTLeo_fn_filters .checkbox').off().on('click',function(){
				
					// our clicked filter
				var element 	= $(this),
					
					// collection wrapper
					parent		= element.closest('.NFTLeo_fn_collection'),
					
					// filter result box
					resultBox	= parent.find('.NFTLeo_fn_result_box'),
				...
				// if clicked item has already clicked and clicked second time
					...
					var index = NFTLeoFilterArray.indexOf(id);
					if(index !== -1){
						NFTLeoFilterArray.splice(index, 1);
					}
				}
				
				
				creativesNFTLeo.recallGridAfterFiltering();
		...
		recallGridAfterFiltering: function(clear){
			...
			if(NFTLeoFilterArray.length === 0){
				filters = '*';
			}
			// if there has one or more selected filters
			else{
				$.each(NFTLeoFilterArray, function(index,value){
					filters += '.id'+value;
				});
			}
			console.log(NFTLeoFilterArray);
			// run isotope after filter has been clicked
			$grid.isotope({ filter: filters });
		},
		
		removeFilter: function(){
			$('.NFTLeo_fn_result_box .result_item a').off().on('click',function(){
				var e 			= $(this),
					parent		= e.closest('.NFTLeo_fn_collection'),
					resultItem	= e.closest('.result_item'),
					resultBox	= parent.find('.NFTLeo_fn_result_box'),
					id 			= resultItem.data('id'),
					filterCount = resultBox.find('.filter_count span');
				resultItem.remove();
				parent.find('.NFTLeo_fn_filters .checkbox[data-id="'+id+'"]').removeClass('selected').find('input[type="checkbox"]').prop('checked','');
				filterCount.text(parseInt(filterCount.text())-1);
				if(resultBox.find('.result_item').length === 0){
					resultBox.find('.clear_all').remove();
				}
			...
			$('.NFTLeo_fn_result_box .clear_all').off().on('click',function(){
				var e 			= $(this),
					parent		= e.closest('.NFTLeo_fn_collection'),
					filterBox	= parent.find('.NFTLeo_fn_filters'),
					resultBox	= parent.find('.NFTLeo_fn_result_box'),
					filterCount = resultBox.find('.filter_count span');
				filterCount.text(0);
				...
				creativesNFTLeo.recallGridAfterFiltering('clear');
				return false;
			});
		},
		...
		movingTextForNav: function(){
			...
			if(overlay.length){
				if(!$('.NFTLeo_fn_moving_text').length){
					$('body').append('<div class="NFTLeo_fn_moving_text">Close</div>');
				}
				var movingBox		= $('.NFTLeo_fn_moving_text');
		...
		anchor: function(){
			$('.NFTLeo_fn_down').on('click',function(){
				var e 		= $(this),
					href 	= e.attr('href');
		...
		navFixer: function(){
			var footerHeight = $('.NFTLeo_fn_nav .nav_footer').outerHeight();
			$('.NFTLeo_fn_nav .nav_content').css({height: ($(window).height() - footerHeight) + 'px' });
		},
		
		movingBlog: function(){
			var blog = $('.NFTLeo_fn_moving_blog');
			if(blog.length){
				if(!$('.NFTLeo_fn_moving_box').length){
					$('body').append('<div class="NFTLeo_fn_moving_box">');
				}
				var movingBox		= $('.NFTLeo_fn_moving_box');
				var list			= $('.NFTLeo_fn_moving_blog .item');	
				list.on('mouseenter',function(event){
					...
					if(!element.hasClass('active')){
						...
						movingBox.addClass('active');
						var imgURL		= element.find('.blog-alt-img').attr('src');
		...
		timeLine: function(){
			var allContentItems 	= $('.NFTLeo_fn_timeline .timeline_item');
			var progressLine		= $('.NFTLeo_fn_timeline .progress_line');
			var allProgressItems 	= $('.NFTLeo_fn_timeline .progress_line li');
			var activeProgressItem 	= $('.NFTLeo_fn_timeline .progress_line .active');
			var activeLine 			= $('.NFTLeo_fn_timeline .progress_line .active_line');
			var allProgressButtons	= $('.NFTLeo_fn_timeline .progress_line a');
			var initialSpace		= 110;
			var spaceBetweenItems	= 230;
			
			creativesNFTLeo.timelineClasses(allContentItems,allProgressItems,'initial');
			...
			allProgressButtons.off().on('click',function(){
				...
				if(!p.hasClass('active')){
					var timeline 	= e.closest('.NFTLeo_fn_timeline');
					var activeIndex = p.data('index');
					creativesNFTLeo.timelineClasses(allContentItems,allProgressItems,activeIndex);
			...
			var nextActive = true;
			var prevActive = true;
			var prevButton = $('.NFTLeo_fn_timeline .nav_prev');
			var nextButton = $('.NFTLeo_fn_timeline .nav_next');
		...
		timelineClasses: function(allContentItems,allProgressItems,activeIndex){
			...
			isActive 	= false;
			if(activeIndex === 'initial'){
				extraActiveIndex = $('.NFTLeo_fn_timeline .progress_line .active').data('index');
			}else{
		...
		headerTrigger: function(){
			var trigger 		= $('.NFTLeo_fn_header .trigger');
			var navOverlay		= $('.nav_overlay');
			var rightNav		= $('.NFTLeo_fn_nav');
			var trigger2 		= rightNav.find('.trigger');
			var links			= $('.NFTLeo_fn_nav .nav_menu a');
			var buttons			= $('.NFTLeo_fn_nav .nav_buttons');
			var footer			= $('.NFTLeo_fn_nav .nav_footer');

			var menuItems		= $('.NFTLeo_fn_nav .nav_menu > ul > li');
